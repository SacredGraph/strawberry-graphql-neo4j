...
def build_cypher_selection(
        ...
        if not (rel_type is None):
            return build_cypher_selection(
                (
                    ...
                    f"({nested_variable}:{inner_schema_type.origin.__name__} {subquery_args}) | {nested_variable} "
    ...
    if rel_type is None:
        return build_cypher_selection(
            (
                f"{initial} {field_name}: {'head(' if not is_array_type(field_type) else ''}[{field_name} in {variable_name}.{field_name} | {field_name} {{{ build_cypher_selection( initial='', selections=getattr(head_selection.selection_set, 'selections', []), variable_name=field_name, schema_type=inner_schema_type, resolve_info=resolve_info) }}}]{')' if not is_array_type(field_type) else ''} {comma_if_tail}"
            ),
            **tail_params,
        )
