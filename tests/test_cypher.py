import unittest
from tests.helpers.cypher_test_helpers import (
    run_test,
    augmented_schema_cypher_test_runner,
)
...
class TestSchema(unittest.TestCase):
    ...
    def augmented_schema_test(self, graphql_query, expected_cypher_query, params=None):
        results = augmented_schema_cypher_test_runner(
            self, graphql_query, expected_cypher_query, params
        )
        if results.errors is not None:
            raise results.errors[0]
    ...
    def test_cypher_projection_skip_limit(self):
        ...
        """
        expected_cypher_query = (
            'MATCH (movie:Movie {title: "River Runs Through It, A"}) RETURN movie { .title ,'
    ...
    def test_query_single_object_relation(self):
        ...
        """
        expected_cypher_query = (
            'MATCH (movie:Movie {movieId: "3100"}) '
            "RETURN movie { .title ,"
    ...
    def test_query_single_object_array_of_objects_relations(self):
        ...
        """
        expected_cypher_query = (
            'MATCH (movie:Movie {movieId: "3100"}) RETURN movie { .title ,'
            "actors: [(movie)<-[:ACTED_IN]-(movie_actors:Actor {}) | movie_actors { .name }] ,"
            "filmedIn: head([(movie)-[:FILMED_IN]->(movie_filmedIn:State {}) | "
            "movie_filmedIn { .name }]) } AS movie SKIP 0"
        )
    ...
    def test_query_by_internal_neo4j_id(self):
        ...
        """
        expected_cypher_query = (
            "MATCH (movie:Movie {}) WHERE ID(movie)=0 "
            "RETURN movie { .title , .year } AS movie SKIP 0"
        )
        self.cypher_test(graphql_query, expected_cypher_query)
        self.augmented_schema_test(graphql_query, expected_cypher_query)
    ...
    def test_query_by_internal_neo4j_id_and_other_params_before_id(self):
        ...
        """
        expected_cypher_query = (
            'MATCH (movie:Movie {title: "River Runs Through It, A"}) WHERE ID(movie)=0 '
            "RETURN movie { .title , .year } AS movie SKIP 0"
        )
        self.cypher_test(graphql_query, expected_cypher_query)
        self.augmented_schema_test(graphql_query, expected_cypher_query)
    ...
    def test_query_by_internal_neo4j_id_and_other_params_after_id(self):
        ...
        """
        expected_cypher_query = (
            "MATCH (movie:Movie {year: 2010}) WHERE ID(movie)=0 "
            "RETURN movie { .title , .year } AS movie SKIP 0"
        )
        self.cypher_test(graphql_query, expected_cypher_query)
        self.augmented_schema_test(graphql_query, expected_cypher_query)
    ...
    def test_cypher_subquery_filters(self):
        ...
        """
        expected_cypher_query = (
            'MATCH (movie:Movie {title: "River Runs Through It, A"}) RETURN movie '
    ...
    def test_handle_cypher_directive_on_get_type(self):
        graphql_query = """
        {
        ...
        """
        expected_cypher_query = (
            'WITH apoc.cypher.runFirstColumn("MATCH (g:Genre) WHERE toLower(g.name) '
    ...
    def test_handle_cypher_directive_on_get_type(self):
        ...
        """
        expected_cypher_query = (
            'CALL apoc.cypher.doIt("CREATE (g:Genre) SET g.name = $name RETURN g", '
    ...
    def test_create_node_get_type(self):
        graphql_query = """
        mutation someMutation {
        ...
        """
        expected_cypher_query = (
            "CREATE (movie:Movie) SET movie = $params RETURN movie {_id: ID(movie), .title ,"
    ...
    def test_add_relation_mutation(self):
        ...
        """
        expected_cypher_query = (
            "MATCH (movie:Movie {movieId: $moviemovieId}) "
    ...
    def test_add_relation_mutation_with_graphql_variables(self):
        ...
        self.cypher_test(
            graphql_query, expected_cypher_query, params={"movieParam": "123"}
        )
    ...
    def test_handle_graphql_variables_in_nested_selection_first(self):
        ...
        """
        expected_cypher_query = (
            "MATCH (movie:Movie {year: 2016}) RETURN movie { .title , .year ,"
            'similar: [ movie_similar IN apoc.cypher.runFirstColumn("WITH {this} AS this '
            'MATCH (this)--(:Genre)--(o:Movie) RETURN o", {this: movie, first: 3, offset: 0}, '
            "true) | movie_similar { .title }][..3] } AS movie SKIP 0"
        )
        self.cypher_test(
            graphql_query, expected_cypher_query, params={"year": 2016, "first": 3}
        )
        self.augmented_schema_test(
            graphql_query, expected_cypher_query, params={"year": 2016, "first": 3}
        )
    ...
    def test_handle_graphql_variables_in_nested_selection_cypher(self):
        ...
        self.cypher_test(
            graphql_query,
            expected_cypher_query,
            params={"year": 2016, "first": 3, "scale": 5},
        )
        self.augmented_schema_test(
            graphql_query,
            expected_cypher_query,
    ...
    def test_return_internal_node_id(self):
        ...
        """
        expected_cypher_query = (
            "MATCH (movie:Movie {year: 2016}) RETURN movie {_id: ID(movie), .title , .year ,"
    ...
    def test_enum_as_scalar(self):
        ...
        """
        expected_cypher_query = (
            "MATCH (book:Book {}) RETURN book { .genre } AS book SKIP 0"
        )
        self.augmented_schema_test(
            graphql_query,
            expected_cypher_query,
